...
@endif
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        @foreach($events as $event)
            <div class="card flex flex-col justify-between bg-white shadow-md rounded-lg overflow-hidden transform hover:scale-[102%] transition-transform duration-200 ease-in-out">
<div class="p-4">
    <h3 class="text-xl font-bold">{{ $event->name }}</h3>
    <p class="text-gray-600">{{ $event->description }}</p>
    <p class="text-sm text-gray-500">Starts: {{ \Carbon\Carbon::parse($event->start_time)->diffForHumans() }}</p>
    <p class="text-sm text-gray-500">Ends: {{ \Carbon\Carbon::parse($event->end_time)->diffForHumans() }}</p>
    <p class="text-sm text-gray-500">Duration: {{ \Carbon\Carbon::parse($event->start_time)->diffInHours($event->end_time) }} hours {{ \Carbon\Carbon::parse($event->start_time)->addHours(\Carbon\Carbon::parse($event->start_time)->diffInHours($event->end_time))->diffInMinutes($event->end_time) }} minutes</p>
</div>
                <div class="px-4 pb-4">
